...
* **Intelligence Layering**: Analysis must be performed as an 'Append Only' operation.We add new columns or flields, never replacing original data.
...
* **Zero Hardcoding Policy**: Prompts, examples, and instructions must never contain dataset-specific entities (e.g., specific country names or publications). Use generic placeholders or inject context dynamically via the project `manifest.json`.
* **Mandatory Manifest Parameters**: Every project `manifest.json` **MUST** include highly specified contextual parameters, specifically `project_name`, `target_topic`, `target_region`, and `topic_definition`. These are enforced at startup in stance discovery scripts to ensure that prompts are perfectly situated in geographical and thematic context without manual code intervention.
* **Theme Definition Descriptions**: The themes schema (`themes_definition.csv`) **MUST** include an explicit `Description` column providing semantic guidelines and scope constraints for each theme. The inductive stance discovery engine dynamically reads these descriptions to anchor LLM clustering and prevent narrative smearing across unrelated strategic pillars.
...
* **Agent Error Resolution Mandate**: If an AI agent or developer encounters a runtime error:
  ...
  3. Instead, the agent **MUST** present the user with a detailed breakdown explaining:
     ...
     * The **Replicability Impact**: A detailed assessment of how the fix affects the replicability, output volumes, or data structures of previous project runs (e.g. `Kagame`, `ANC-IRAN`, or `Lighthouse`).
